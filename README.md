...
| 对比项       | 官方客户端（闭源）               | 官方开源版                 | ZCode Open Audit（本仓库）                                 |
| ------------ | -------------------------------- | -------------------------- | ---------------------------------------------------------- |
| 监控与遥测   | 全套默认开启，开关管不到打包上传 | 与闭源版相同               | **全部移除**（约 2.6 万行），并加防回归检查                |
| 仓库上传逻辑 | 有（直到 2026-09-18 被曝光）     | 已移除（自 2026-09-21 起） | 已移除                                                     |
| 历史版本回溯 | 旧版本下载链接已下架             | 旧版本下载链接已下架       | **保留全部历史版本与提交记录**，供审计回溯                 |
| 构建透明度   | 官方二进制，构建不可复现         | 不提供公开构建             | **GitHub Actions 从仓库源码透明构建**，产物随 Release 发布 |
| Issue 与共建 | 不开放                           | 关闭                       | **开放**（Issue 与 Discussions），欢迎共建讨论             |
| 安装包签名   | 已签名                           | 不提供安装包               | 未签名（附一键放行命令）                                   |
