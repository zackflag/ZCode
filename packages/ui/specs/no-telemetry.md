...
验收：`pnpm test:no-telemetry` 的静态出口防回归检查通过，业务包装回调保留，购买 WebView 与升级组件保持删除；该检查已纳入 `pnpm verify:pre-push`。根目录 pnpm typecheck 成功、pnpm lint 为 0 error；架构无新增违反。交互验收场景为发送/队列确认、自动化创建、设置保存、错误边界重试和主动反馈；官方服务开关与模型 Provider 分别验收，未实际运行 E2E 必须在报告中说明。
